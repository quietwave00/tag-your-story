package com.tagnote.batch.job.step.config;

import com.tagnote.batch.item.CustomItemWriter;
import com.tagnote.batch.job.parameter.DeleteFileJobParameter;
import com.tagnote.batch.mapper.FileListRowMapper;
...
@Configuration
public class DeleteFileFromStorageStepConfig {
    private static final String STEP_NAME = "DELETE_FILE_FROM_STORAGE";
    ...
    public Step deleteFile(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        log.info("deleteFileFromStorage() Executed");
    ...
    /**
     * 파일 스토리지로 파일 삭제 요청을 한다.
     */
    @Bean(WRITER_NAME)
    @StepScope
    public ItemWriter<List<String>> fileItemWriter() {
        return new CustomItemWriter(fileStorageClient);
    }
